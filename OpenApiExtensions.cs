namespace Tooling.Plumbing
{
    public static class OpenApiExtensions
    {
        ...
        public static OpenApiOptions AddOpenApiAuthentication(this OpenApiOptions options)
        {
            options.AddDocumentTransformer((document, context, cancellationToken) =>
            {
                ...
                var securityScheme = new OpenApiSecurityScheme
                {
                    ...
                    Name = "Authorization",
                    Scheme = schemeName.ToLower(), // O schema geralmente é passado em minúsculo
                    In = ParameterLocation.Header,
                    BearerFormat = "JWT",
                    Description = "Entre com um token válido do acesso cidadão."
                };
        ...
        // =========================================================================
        // PIPELINES (Configuração de UI)
        // =========================================================================
        public static WebApplication UseNativeOpenApiUI(this WebApplication app)
        {
            var docs = app.Configuration.GetSection("SwaggerDocs").Get<IList<DocInfoDto>>() ?? new List<DocInfoDto>();

            // IMPORTANTE: Mapeia os arquivos JSON gerados pelo .NET 10
            // Sem isso, a rota /openapi/{guid}.json retornará 404.
            app.MapOpenApi();
            ...
            app.UseSwaggerUI(c =>
            {
                foreach (var doc in docs)
                {
                    // Aponta para os JSONs gerados pelo MapOpenApi nativo
                    c.SwaggerEndpoint($"/openapi/{doc.Guid}.json", $"{doc.Title} Versão {doc.Version}");
                }
            });
